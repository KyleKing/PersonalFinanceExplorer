...
class PXDemoApp:
    ...
    def createLayout(self):
        """Create layout."""
        # Style tabs compactly
        tabStyle = {
            'padding-top': '10px',
            'padding-bottom': '10px',
        }
        tabStyleSelec = copy.copy(tabStyle)
        tabStyleSelec['border-left'] = '3px solid #119DFF'
        self.app.layout = html.Div(children=[
            html.Div(children=[
                dcc.Tabs(
                    id='tabs-select', value=self.TABS[0].title, vertical=True,
                    children=[
                        dcc.Tab(label=_tab.title, value=_tab.title, style=tabStyle, selected_style=tabStyleSelec) for _tab in self.TABS
                    ],
                    style={'width': '100%'},
